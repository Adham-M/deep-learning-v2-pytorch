...
class NeuralNetwork(object):
    ...
    def forward_pass_train(self, X):
        ...
        hidden_inputs = np.dot(X,self.weights_input_to_hidden) #(1,3).(3,2) -> (1,2) # signals into hidden layer
        hidden_outputs = self.activation_function(hidden_inputs) # (1,2) # signals from hidden layer

        # TODO: Output layer - Replace these values with your calculations.
        final_inputs = np.dot(hidden_outputs,self.weights_hidden_to_output) # (1,1) # signals into final output layer
    ...
    def backpropagation(self, final_outputs, hidden_outputs, X, y, delta_weights_i_h, delta_weights_h_o):
        ...
        hidden_error_term = np.reshape(hidden_error_term, (1, self.hidden_nodes))
        
        delta_weights_i_h += np.dot(X.T,hidden_error_term)
        
        
        # Weight step (hidden to output)
        hidden_outputs = np.reshape(hidden_outputs, (1, self.hidden_nodes))
        output_error_term = np.reshape(output_error_term, (1, self.output_nodes))
    ...
    def run(self, features):
        ...
        X = np.reshape(features, (1, self.input_nodes))
